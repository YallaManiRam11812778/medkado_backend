...
@frappe.whitelist()
def maps_page(location=None):
	if location==None:location = frappe.db.get_all("Medkado User",{"name":frappe.session.user},pluck="location")[0]
	list_of_hospitals = frappe.db.get_all("Medkado Hospitals",{"district":location},["location","hospital_short_name","hospital_name"])
	if not len(list_of_hospitals)>0:
